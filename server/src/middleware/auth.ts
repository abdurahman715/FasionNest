...
export function requireAuth(req: Request, _res: Response, next: NextFunction) {
  ...
  if (!userId) {
    return next(
      new AppError(401, "User is not logged in.Means unauthenticated user"),
    );
  }
  next();
}
...
export async function getDbUserFromReq(req: Request) {
  const { userId } = getAuth(req);
  if (!userId) {
    throw new AppError(401, "User is not logged in.Means unauthenticated user");
  }
...
//Admin gate (user loggedIn + admin access)
